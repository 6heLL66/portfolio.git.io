...
function drawRect() {
	...
	for (let i = 0; i < rectangles.length; i++) {
		...
		if (i == rectangles.length - 1) {
			...
			else if (rectangles[i].posx == 0) turnRect(rectangles[i].posx, rectangles[i].posy, 'right')
			else if (rectangles[i].posy == 0) turnRect(rectangles[i].posx, rectangles[i].posy, 'bottom')
			else if (rectangles[i].posy == canvas.height - rectHeight /                                           2) turnRect(rectangles[i].posx, rectangles[i].posy, 'top')
		}
		...
		else if (rectangles[i].turn == 'left') {
			rectangles[i].posx -= rectSpeed	
		}
		else if (rectangles[i].turn == 'bottom') {
			rectangles[i].posy += rectSpeed
		}
		else rectangles[i].posx += rectSpeed
...
function turnRect(posx,posy,side) {
	...
	for (let i = 0; i < rectangles.length - 1; i++) {
		if (rectangles[i].stack != '') rectangles[i].stack.push({ side, posx, posy })
		else rectangles[i].stack = [{ side, posx, posy }]
	}	
}
...
document.onkeydown = function(e) {
		...
		else if (e.key == 's' || e.key == 'ы') turnRect(posx, posy, 'bottom')
		else if (e.key == 'a' || e.key == 'ф') turnRect(posx, posy, 'left')
		else if (e.key == 'd' || e.key == 'в') turnRect(posx, posy, 'right')
...
canvas.onclick = function(e) {
	let element = rectangles[rectangles.length - 1]

	if ((element.turn == 'right' || element.turn == 'left') && e.clientY - 2 < element.posy) turnRect(element.posx, element.posy, 'top')
	else if ((element.turn == 'right' || element.turn == 'left') && e.clientY - 2 > element.posy) turnRect(element.posx, element.posy, 'bottom')
	else if ((element.turn == 'top' || element.turn == 'bottom') && e.clientX - 2 > element.posx) turnRect(element.posx, element.posy, 'right')
	else if ((element.turn == 'top' || element.turn == 'bottom') && e.clientX - 2 < element.posx) turnRect(element.posx, element.posy, 'left')
}
...
function addPart() {
	...
	else if (rectangles[0].turn == 'right') {
		posx = rectangles[0].posx - rectWidth - rectMargin
		posy = rectangles[0].posy
	}
	else if (rectangles[0].turn == 'top') {
		posx = rectangles[0].posx
		posy = rectangles[0].posy + rectHeight + rectMargin
	}
	else if (rectangles[0].turn == 'bottom'){
		posx = rectangles[0].posx
		posy = rectangles[0].posy - rectHeight - rectMargin
	}
