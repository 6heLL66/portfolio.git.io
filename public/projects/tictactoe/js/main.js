...
function botStep() {
	...
	if (!checkWin() && !checkLoose()) {
		while (true) {
			...
			if (state[num] != "x") {
				if ((state[1] != "x"  || state[3] != "x" || state[7] != "x" || state[9] != "x") 
					&& (num == 2 || num == 4 || num == 5 || num == 6 || num == 8)) continue
				state[num] = 'o'
				setToe(num)
				break
			}
		...
		for(let i = 0; i < check.length; i++) {
			...
			if(v != 0 && c == 2) {
				...
				if (check[i][0] <= 3) {
					...
					y1 = Math.floor(check[i][0] / 3 - .001)
					x1 = check[i][0] - (y1) * 3
					y2 = Math.floor(v / 3 - .001)
					x2 = v - y1 * 3
				}
				ctx.moveTo(x1, y1)
				ctx.lineTo(x2, y2)
