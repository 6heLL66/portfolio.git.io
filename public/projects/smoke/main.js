...
let up = setInterval(updatePixels, 20)
let h = 0
let active = 1
let	pixels = []
...
function setPixels(posX, posY ,min, max, n) {
	...
	for (let i = 0; i < pixels.length; i++) {
		...
		let vel =  Math.random() * (max.vel - min.vel) + min.vel
		let	ang = Math.random() * (max.ang - min.ang) + min.ang
		let	rgba = [Math.round(Math.random() * (max.r - min.r) + min.r),
					Math.round(Math.random() * (max.g - min.g) + min.g),
					Math.round(Math.random() * (max.b - min.b) + min.b),
					Math.round(Math.random() * (max.a - min.a) + min.a)]
		let obj = {
			color: rgba,
...
function setImages(posX, posY, min, max, n) {
	...
	let	size =  Math.round(Math.random() * (1 - 3) + 3)
	...
	if (active == 1) {
		for (let i = 0; i < h; i++) {
			if (pixels[i].alive == false) continue
				...
				let	y = Math.round(pixels[i].posY)
